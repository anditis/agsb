...
def generate_links(domain, port_vm_ws, uuid_str):
    ...
    for ip, port_cf in cf_ips_tls.items():
        ps_name = f"ANDS-50-{hostname}-{ip.split('.')[2]}-{port_cf}"
        config = {
    ...
    for ip, port_cf in cf_ips_http.items():
        ps_name = f"ANDS-H-50-{hostname}-{ip.split('.')[2]}-{port_cf}"
        config = {
            "ps": ps_name, "add": ip, "port": port_cf, "id": uuid_str, "aid": "0",
            "net": "ws", "type": "none", "host": domain, "path": ws_path_full,
            "tls": "" # 非TLS，此项为空
    ...
    # TLS Direct
    direct_tls_config = {
        "ps": f"ANDS-50-{hostname}-Direct-{domain[:15]}-443", 
    ...
    # HTTP Direct
    direct_http_config = {
        "ps": f"ANDS-H-50-{hostname}-Direct-{domain[:15]}-80",
...
def install(args):
    ...
    if final_domain:
        ...
        for ip, port_cf in cf_ips_tls.items():
            config = {
                "ps": f"ANDS-50-{hostname}-{ip.split('.')[2]}-{port_cf}", "add": ip, "port": port_cf, "id": uuid_str, "aid": "0",
        ...
        for ip, port_cf in cf_ips_http.items():
            config = {
                "ps": f"ANDS-H-50-{hostname}-{ip.split('.')[2]}-{port_cf}", "add": ip, "port": port_cf, "id": uuid_str, "aid": "0",
                "net": "ws", "type": "none", "host": final_domain, "path": ws_path_full,
                "tls": ""
            }
            all_links.append(generate_vmess_link(config))
        direct_tls_config = {
            "ps": f"ANDS-50-{hostname}-Direct-{final_domain[:15]}-443",
        ...
        all_links.append(generate_vmess_link(direct_tls_config))
        direct_http_config = {
            "ps": f"ANDS-H-50-{hostname}-Direct-{final_domain[:15]}-80",
